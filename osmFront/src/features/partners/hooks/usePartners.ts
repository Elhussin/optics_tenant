...
export function usePartners() {
    ...
    const getPartner = useCallback(async (id: number): Promise<Partner | null> => {
        try {
    ...
    const createPartner = useCallback(async (partner: Partial<Partner>) => {
        setLoading(true);
        try {
    ...
    const updatePartner = useCallback(async (id: number, partner: Partial<Partner>) => {
        setLoading(true);
        try {
            const data = await api.customRequest('crm_partners_partial_update', { id, ...partner });
            await fetchPartners();
            return data;
        } catch (err: any) {
...
export function usePartnerCustomers() {
    ...
    const fetchLinks = useCallback(async (params?: Record<string, any>) => {
        ...
        setError(null);
        try {
            const data = await api.customRequest('crm_partner_customers_list', params);
...
export function useInsuranceClaims() {
    ...
    const createClaim = useCallback(async (claim: ClaimCreate) => {
        setLoading(true);
        try {
            const data = await api.customRequest('crm_insurance_claims_create', claim);
            await fetchClaims();
            return data;
        } catch (err: any) {
    ...
    const updateClaimStatus = useCallback(async (
        ...
        setLoading(true);
        try {
            const result = await api.customRequest('crm_insurance_claims_partial_update', {
                id: claimId,
                status,
                ...data,
            });
    ...
    const submitClaim = useCallback(async (claimId: number) => {
        return updateClaimStatus(claimId, 'submitted', {
        });
    }, [updateClaimStatus]);

    const approveClaim = useCallback(async (claimId: number, approvedAmount: string) => {
        return updateClaimStatus(claimId, 'approved', { approved_amount: approvedAmount });
    }, [updateClaimStatus]);

    const rejectClaim = useCallback(async (claimId: number, reason: string) => {
        return updateClaimStatus(claimId, 'rejected', { rejection_reason: reason });
    }, [updateClaimStatus]);
...
export function usePartnerStatement(partnerId: number) {
    ...
    const fetchStatement = useCallback(async (startDate?: string, endDate?: string) => {
        ...
        setError(null);
        try {
            const params: Record<string, string> = { partner_id: String(partnerId) };
            if (startDate) params.start_date = startDate;
            if (endDate) params.end_date = endDate;

            const data = await api.customRequest('crm_partners_statement_retrieve', params);
            setStatement(data);
            return data;
        } catch (err: any) {
