...
export function TransfersList() {
  ...
  const handleQuickAction = async (
    id: number,
    action: "submit" | "approve" | "ship",
  ) => {
    try {
      if (action === "submit") {
        await submitMutation.mutation.mutateAsync({ id });
        safeToast(
        ...
        await approveMutation.mutation.mutateAsync({ id });
        safeToast(
        ...
        await shipMutation.mutation.mutateAsync({ id });
        safeToast(
      ...
      handleError(
        error,
        t("transfers.messages.actionError") || "Action failed",
      );
    }
  };
  ...
  const executeReceive = async () => {
    ...
    try {
      ...
      handleError(error, t("transfers.messages.receiveError"));
    } finally {
                  ...
                  (transfer: StockTransfer, index: number) => (
                    ...
                    >
                        ...
                        <div className="flex gap-2">
                          ...
                          {transfer.status === "pending" && (
                            <ActionButton
                              ...
                              size="sm"
                              icon={<Check size={16} />}
                              label={t("transfers.actions.submit") || "Submit"}
                              onClick={() =>
                                handleQuickAction(transfer.id, "submit")
                              }
                          ...
                          {transfer.status === "submitted" && (
                            <div className="flex gap-2">
                              <ActionButton
                                ...
                                icon={<Check size={16} />}
                                label={
                                  t("transfers.actions.approve") || "Approve"
                                }
                                onClick={() =>
                                  handleQuickAction(transfer.id, "approve")
                                }
                                disabled={approveMutation.mutation.isPending}
                              />
                              {/* Sometimes validation requires shipping separately or auto? Assuming manual ship flow */}
                              <ActionButton
                                variant="info"
                                size="sm"
                                icon={<Package size={16} />} // Using Package icon for ship
                                label={t("transfers.actions.ship") || "Ship"}
                                onClick={() =>
                                  handleQuickAction(transfer.id, "ship")
                                }
                      ...
                      {transfer.items && transfer.items.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-border-main/30 bg-gray-50/50 dark:bg-gray-800/30 -mx-4 -mb-4 px-4 py-3 rounded-b-xl">
                          <p className="text-xs font-semibold text-secondary mb-2">
                            {t("transfers.list.includes") || "Includes:"}
                          </p>
                          <div className="flex flex-wrap gap-2">
                            {transfer.items.slice(0, 3).map((item, idx) => (
                              <span
                                key={item.id || idx}
                                className="text-xs bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 px-2 py-1 rounded-md text-main"
                              >
                                {item.variant_name}{" "}
                            ...
                            {transfer.items.length > 3 && (
                              <span className="text-xs bg-gray-100 dark:bg-gray-800 text-secondary px-2 py-1 rounded-md">
                                +{transfer.items.length - 3}{" "}
                                {t("transfers.list.more") || "more"}
                              </span>
                            )}
                          </div>
                        </div>
