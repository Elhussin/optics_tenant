...
def get_variant_stock_summary(variant):
    ...
    if hasattr(variant, 'stocks'):
        ...
        for inventory in inventories:
            ...
            if inventory.available_quantity <= inventory.reorder_level and inventory.available_quantity > 0:
...
def find_nearest_branch_with_stock(variant, user_location=None, min_quantity=1):
    ...
    available_stocks = [
        s for s in available_stocks if s.available_quantity >= min_quantity]
...
def can_fulfill_order_across_branches(items_dict):
    ...
    for variant, needed_quantity in items_dict.items():
        ...
        available_stocks = Stock.objects.filter(
            variant=variant).select_related('branch')
        ...
        available_stocks = sorted(
            available_stocks, key=lambda s: s.available_quantity, reverse=True)
