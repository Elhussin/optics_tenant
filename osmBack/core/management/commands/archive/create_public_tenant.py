...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--domain', type=str, default='localhost',
                            help='Domain name for the public tenant')
        parser.add_argument('--name', type=str,
                            default='Main Site', help='Name of the tenant')
        parser.add_argument('--paid_until', type=str,
                            default='2030-12-31', help='Paid until date in YYYY-MM-DD')
        parser.add_argument('--trial', action='store_true',
                            help='Set tenant as on trial')
        parser.add_argument(
            '--password', type=str, help='Password for the public superuser (prompts if missing)')
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(self.style.SUCCESS(
                f'✅ Public tenant created: {name}'))
        else:
            self.stdout.write(self.style.WARNING(
                f'ℹ️ Public tenant "{name}" already exists. Checking components...'))
        ...
        if domain_created:
            self.stdout.write(self.style.SUCCESS(
                f'✅ Domain {domain} assigned to tenant.'))
        else:
            self.stdout.write(self.style.WARNING(
