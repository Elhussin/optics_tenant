...
//           console.log(data);
//           const detectedCountry = data.country_code?.toLowerCase() || 'sa';
//           const countryInfo = countries.find((c) => c.value === detectedCountry) || countries[0];
//           setCountry(countryInfo.value);
//           setCurrency(countryInfo.value);
//           Cookies.set('country', countryInfo.value, { path: '/' });
//           Cookies.set('currency', countryInfo.value, { path: '/' });
//         });
//     }
//   }, []);
...
export default function LocaleSwitcher() {
  ...
  const handleLanguageChange = (value: string) => {
    ...
    if (pathSegments.length > 1) {
      pathSegments[1] = value; // استبدال رمز اللغة
    } else {
      pathSegments.unshift('', value); // إضافة اللغة إذا لم تكن موجودة
    }
