...
export default function DynamicFormPage() {
  const { name } = useParams(); // ده اسم الـ dynamic route
  const params = useSearchParams(); // لقراءة باقي الإعدادات من URL
  ...
  const title = params.get('title') || 'Create ' + name;
  if (!schemaName || !alias) {
  ...
  return (
    <DynamicFormGenerator
      ...
      title={title } // ممكن تخلي الـ title هو نفس الـ name
    />
  );
}
